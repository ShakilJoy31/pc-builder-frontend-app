...
const Page = () => {
  ...
  useEffect(() => {
    CustomerAPI.handleGettingProducts(1, 'Monitor').then(res => {
      setFetchedData(res);
    });
    if (JSON.parse(localStorage.getItem('Monitor'))) {
      setIsPCBuilder(true);
    }
...
setTimeout(function () {
  ...
  const newParticularMenu = getProduct;
      ...
      if (!isAlreadyInCart) {
          ...
          localStorage.setItem("MonitorForPCBuilding", JSON.stringify(getProduct));
        ...
        <div className='flex justify-between items-center'>
          <h1 className='block mx-auto w-full mt-4'>
            <svg className="gradient-text" width="100%" height="38" xmlns="http://www.w3.org/2000/svg">
              ...
              <text className='lg:text-3xl md:text-2xl text-xl' x="10" y="30" fill="url(#gradient)" textAnchor="start">Monitor</text>
            </svg>
          </h1>
        </div>
